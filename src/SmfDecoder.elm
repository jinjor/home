...
eventListHelp prevStatus prev =
    given (deltaTimeAndData prevStatus)
        (\( prevStatus, e ) ->
            case e of
                ( _, End ) ->
                    succeed (e :: prev)
...
event status first =
    if status // 16 == 8 then
        ...
        succeed
            ...
            |= uint8
    else if status // 16 == 0x0A then
        succeed (KeyPressure (status % 16) first)
            |= uint8
    else if status // 16 == 0x0B then
        succeed (ControlChange (status % 16) first)
            |= uint8
    else if status // 16 == 0x0C then
        succeed (ProgramChange (status % 16) first)
    else if status // 16 == 0x0D then
        succeed (ChannelPressure (status % 16) first)
    else if status // 16 == 0x0E then
        succeed (\msb -> PitchWheelChange (status % 16) 0)
            -- TODO
            |=
                uint8
    else if status // 16 == 0x0F then
        meta first
    else
